    ...
    if (empty($username) || empty($password)) {
        ...
        if ($result->num_rows > 0) {
            ...
            if (password_verify($password, $row['password'])) {
                ...
                $_SESSION['user_id'] = $row['user_id']; // เพิ่ม user_id ลงในเซสชัน

                // เปลี่ยนเส้นทางตามบทบาท
                if ($row['role'] === 'admin') {
                    header('Location: index.php'); // แสดงหน้าแอดมิน
                } elseif ($row['role'] === 'employee') {
                    header('Location: index.php'); // แสดงหน้าพนักงาน
                } elseif ($row['role'] === 'customer') {
                    header('Location: customer_dashboard.php'); // แสดงหน้าผู้ใช้
                } else {
