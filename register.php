...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name) || empty($password) || empty($confirmpassword) || empty($email) || empty($first_name) || empty($last_name) || empty($address)) {
        ...
        if ($password !== $confirmpassword) {
            ...
            if ($exists > 0) {
                ...
                $sql = "INSERT INTO users (username, email, password, first_name, last_name, address) VALUES (?, ?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("ssssss", $name, $email, $hashed_password, $first_name, $last_name, $address);
